interface AppProps {
	svg: string;
}
export function CodePen(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1792 1792'>
			<path
				className='fill-current'
				fill='#5757E0'
				d='m216 1169 603 402v-359L485 989zm-62-144 193-129-193-129v258zm819 546 603-402-269-180-334 223v359zm-77-493 272-182-272-182-272 182zM485 803l334-223V221L216 623zm960 93 193 129V767zm-138-93 269-180-603-402v359zm485-180v546q0 41-34 64l-819 546q-21 13-43 13t-43-13L34 1233q-34-23-34-64V623q0-41 34-64L853 13q21-13 43-13t43 13l819 546q34 23 34 64z'
			/>
		</svg>
	);
}
export function Discord(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'>
			<path
				className='fill-current'
				fill='#5757e0'
				d='M85.22,24.958c-11.459-8.575-22.438-8.334-22.438-8.334l-1.122,1.282c13.623,4.087,19.954,10.097,19.954,10.097c-19.491-10.731-44.317-10.654-64.59,0c0,0,6.571-6.331,20.996-10.418l-0.801-0.962c0,0-10.899-0.24-22.438,8.334c0,0-11.54,20.755-11.54,46.319c0,0,6.732,11.54,24.442,12.101c0,0,2.965-3.526,5.369-6.571c-10.177-3.045-14.024-9.376-14.024-9.376c6.394,4.001,12.859,6.505,20.916,8.094c13.108,2.698,29.413-0.076,41.591-8.094c0,0-4.007,6.491-14.505,9.456c2.404,2.965,5.289,6.411,5.289,6.411c17.71-0.561,24.441-12.101,24.441-12.02C96.759,45.713,85.22,24.958,85.22,24.958z M35.055,63.824c-4.488,0-8.174-3.927-8.174-8.815c0.328-11.707,16.102-11.671,16.348,0C43.229,59.897,39.622,63.824,35.055,63.824z M64.304,63.824c-4.488,0-8.174-3.927-8.174-8.815c0.36-11.684,15.937-11.689,16.348,0C72.478,59.897,68.872,63.824,64.304,63.824z'
			/>
		</svg>
	);
}
export function Dribbble(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
			<path
				className='fill-current'
				fill='#5757E0'
				d='M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,2a7.94,7.94,0,0,1,5.51,2.22A24.93,24.93,0,0,1,12.68,8.4,29.07,29.07,0,0,0,9.81,4.31,7.94,7.94,0,0,1,12,4ZM7.89,5.15A27.16,27.16,0,0,1,10.7,9a25.11,25.11,0,0,1-6,.74H4.34A8,8,0,0,1,7.89,5.15ZM6,17.31A7.9,7.9,0,0,1,4,12c0-.1,0-.2,0-.29h.68a26.67,26.67,0,0,0,7-1c.32.61.62,1.24.91,1.89a14.3,14.3,0,0,0-4.29,2.41l-.3.24A21,21,0,0,0,6,17.31ZM12,20a7.92,7.92,0,0,1-4.47-1.37,17.92,17.92,0,0,1,1.56-1.58l.32-.27a12.63,12.63,0,0,1,4-2.27,32,32,0,0,1,1.4,5A8.08,8.08,0,0,1,12,20Zm4.63-1.49a34.87,34.87,0,0,0-1.28-4.46l.34,0a.25.25,0,0,1,.12,0h.11l.1,0,.48,0a9.43,9.43,0,0,1,3.09.53A7.94,7.94,0,0,1,16.63,18.51ZM16.5,12c-.21,0-.42,0-.62,0a1.56,1.56,0,0,0-.39,0,6.64,6.64,0,0,0-.81.1h-.1c-.29-.67-.59-1.32-.92-2a26.57,26.57,0,0,0,5.13-2.4A8,8,0,0,1,20,12c0,.17,0,.34,0,.51A11.48,11.48,0,0,0,16.5,12Z'
			/>
		</svg>
	);
}
export function Figma(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
			<path
				className='fill-current'
				fill='#5757e0'
				fillRule='evenodd'
				d='M8.49 1.147h7.02c2.42 0 4.39 1.915 4.39 4.214 0 1.344-.674 2.558-1.718 3.333 1.044.776 1.718 1.989 1.718 3.334 0 2.298-1.97 4.213-4.39 4.213a4.467 4.467 0 0 1-2.63-.849v3.302c0 2.299-1.969 4.214-4.39 4.214-2.42 0-4.39-1.915-4.39-4.214 0-1.344.674-2.557 1.718-3.333-1.044-.776-1.718-1.989-1.718-3.333 0-1.345.674-2.558 1.718-3.334C4.774 7.919 4.1 6.705 4.1 5.361c0-2.299 1.97-4.214 4.39-4.214Zm2.63 15.094H8.49c-1.43 0-2.63 1.095-2.63 2.453 0 1.359 1.2 2.453 2.63 2.453 1.431 0 2.63-1.094 2.63-2.453v-2.453Zm1.76-4.213c0-1.359 1.199-2.454 2.63-2.454 1.43 0 2.63 1.095 2.63 2.454 0 1.358-1.2 2.453-2.63 2.453-1.431 0-2.63-1.095-2.63-2.453ZM8.457 9.575c-1.416.016-2.597 1.105-2.597 2.453 0 1.358 1.2 2.453 2.63 2.453h2.63V9.574H8.49l-.033.001Zm2.663-1.761V2.908H8.49c-1.43 0-2.63 1.094-2.63 2.453 0 1.359 1.2 2.453 2.63 2.453h2.63Zm1.76 0h2.63c1.43 0 2.63-1.094 2.63-2.453 0-1.359-1.2-2.453-2.63-2.453h-2.63v4.906Z'
			/>
		</svg>
	);
}
export function GitHub(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
			<path
				className='fill-current'
				fill='#5757E0'
				d='M10.07031,20.50291a1.00008,1.00008,0,0,0-1.18115-.9834c-1.30908.24024-2.96191.27637-3.40137-.958a5.70754,5.70754,0,0,0-1.83691-2.415,1.20073,1.20073,0,0,1-.1665-.10938,1,1,0,0,0-.93067-.64551H2.54883a.99965.99965,0,0,0-1,.99512c-.00391.81543.811,1.33789,1.1416,1.51465a4.4408,4.4408,0,0,1,.92383,1.35937c.36426,1.02344,1.42285,2.57617,4.46582,2.376.001.03516.00195.06836.00244.09863l.00439.26758a1,1,0,0,0,2,0l-.00488-.31836C10.07715,21.4951,10.07031,21.22068,10.07031,20.50291Zm10.667-15.126c.03174-.125.063-.26367.09034-.41992a6.27792,6.27792,0,0,0-.40821-3.293,1.002,1.002,0,0,0-.61572-.58007c-.356-.12012-1.67041-.35645-4.18408,1.25a13.86918,13.86918,0,0,0-6.354,0C6.76221.751,5.45459.9658,5.10205,1.07908a.99744.99744,0,0,0-.63135.584,6.3003,6.3003,0,0,0-.40332,3.35644c.02442.12793.05078.2461.07813.35449A6.26928,6.26928,0,0,0,2.89014,9.20311a8.42168,8.42168,0,0,0,.04248.92187c.334,4.60254,3.334,5.98438,5.42431,6.459-.04345.125-.083.25878-.11816.40039a1.00023,1.00023,0,0,0,1.94238.47851,1.6784,1.6784,0,0,1,.46778-.87793.99947.99947,0,0,0-.5459-1.74512c-3.4541-.39453-4.95362-1.80175-5.1792-4.89843a6.61076,6.61076,0,0,1-.03369-.73828,4.25769,4.25769,0,0,1,.91943-2.71289,3.022,3.022,0,0,1,.1958-.23145.99988.99988,0,0,0,.188-1.02441,3.3876,3.3876,0,0,1-.15527-.55567A4.09356,4.09356,0,0,1,6.1167,3.06346a7.54263,7.54263,0,0,1,2.415,1.17968,1.00877,1.00877,0,0,0,.82764.13282,11.77716,11.77716,0,0,1,6.17285.001,1.00549,1.00549,0,0,0,.83056-.13769,7.572,7.572,0,0,1,2.40528-1.19043,4.03977,4.03977,0,0,1,.0874,1.57812,3.205,3.205,0,0,1-.16895.60743.9999.9999,0,0,0,.188,1.02441c.07715.08691.1543.18066.22363.26855A4.12186,4.12186,0,0,1,20,9.20311a7.03888,7.03888,0,0,1-.0376.77734c-.22021,3.05566-1.72558,4.46387-5.1958,4.85937a1,1,0,0,0-.54541,1.7461,1.63079,1.63079,0,0,1,.46631.9082,3.06079,3.06079,0,0,1,.09229.81934v2.334C14.77,21.2949,14.77,21.78025,14.77,22.00291a1,1,0,1,0,2,0c0-.2168,0-.69238.00977-1.33984V18.31346a4.8815,4.8815,0,0,0-.15479-1.31153,4.25638,4.25638,0,0,0-.11621-.416,6.51258,6.51258,0,0,0,5.44531-6.42383A8.69677,8.69677,0,0,0,22,9.20311,6.13062,6.13062,0,0,0,20.7373,5.37693Z'
			/>
		</svg>
	);
}
export function LinkedIn(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
			<path
				className='fill-current'
				fill='#5757E0'
				d='M17.5,8.999a5.41868,5.41868,0,0,0-2.56543.64453A.99918.99918,0,0,0,14,8.999H10a.99943.99943,0,0,0-1,1v12a.99942.99942,0,0,0,1,1h4a.99942.99942,0,0,0,1-1v-5.5a1,1,0,1,1,2,0v5.5a.99942.99942,0,0,0,1,1h4a.99942.99942,0,0,0,1-1v-7.5A5.50685,5.50685,0,0,0,17.5,8.999Zm3.5,12H19v-4.5a3,3,0,1,0-6,0v4.5H11v-10h2v.70313a1.00048,1.00048,0,0,0,1.78125.625A3.48258,3.48258,0,0,1,21,14.499Zm-14-12H3a.99943.99943,0,0,0-1,1v12a.99942.99942,0,0,0,1,1H7a.99942.99942,0,0,0,1-1v-12A.99943.99943,0,0,0,7,8.999Zm-1,12H4v-10H6ZM5.01465,1.542A3.23283,3.23283,0,1,0,4.958,7.999h.02832a3.23341,3.23341,0,1,0,.02832-6.457ZM4.98633,5.999H4.958A1.22193,1.22193,0,0,1,3.58887,4.77051c0-.7461.55957-1.22852,1.42578-1.22852A1.2335,1.2335,0,0,1,6.41113,4.77051C6.41113,5.5166,5.85156,5.999,4.98633,5.999Z'
			/>
		</svg>
	);
}
export function Cancel(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 47.5 47.5'>
			<path
				className='fill-current'
				fill='#5757e0'
				transform='matrix(1.25 0 0 -1.25 0 47.5)'
				d='m23.238 18.996 9.883 9.883a3 3 0 1 1-4.242 4.242l-9.883-9.883-9.883 9.883a3 3 0 1 1-4.242-4.243l9.883-9.882-9.907-9.907A3 3 0 0 1 9.09 4.847l9.906 9.906 9.883-9.882A2.992 2.992 0 0 1 31 3.993a3 3 0 0 1 2.121 5.121l-9.883 9.882Z'
			/>
		</svg>
	);
}
export function Download(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
			<path
				className='fill-current'
				fill='#5757e0'
				d='M21,14a1,1,0,0,0-1,1v4a1,1,0,0,1-1,1H5a1,1,0,0,1-1-1V15a1,1,0,0,0-2,0v4a3,3,0,0,0,3,3H19a3,3,0,0,0,3-3V15A1,1,0,0,0,21,14Zm-9.71,1.71a1,1,0,0,0,.33.21.94.94,0,0,0,.76,0,1,1,0,0,0,.33-.21l4-4a1,1,0,0,0-1.42-1.42L13,12.59V3a1,1,0,0,0-2,0v9.59l-2.29-2.3a1,1,0,1,0-1.42,1.42Z'
			/>
		</svg>
	);
}
export function Hamburger(props: AppProps) {
	const { svg } = props;
	const hamburgerPathStyle = `
		transition-[stroke-dashoffset,_stroke-dasharray]
		duration-500
	`;
	return (
		<svg
			className={`
				${svg}
				scale-125
			`}
			xmlns='http://www.w3.org/2000/svg'
			viewBox='0 0 100 100'
			fill='none'
		>
			<path
				className={`
					stroke-current
					${hamburgerPathStyle}
					path1
				`}
				stroke='#5757e0'
				strokeWidth='6.4px'
				d='M 20,29.000046 H 80.000231 C 80.000231,29.000046 94.498839,28.817352 94.532987,66.711331 94.543142,77.980673 90.966081,81.670246 85.259173,81.668997 79.552261,81.667751 75.000211,74.999942 75.000211,74.999942 L 25.000021,25.000058'
			/>
			<path
				className={`
					stroke-current
					${hamburgerPathStyle}
					path2
				`}
				stroke='#5757e0'
				strokeWidth='6.4px'
				d='M 20,50 H 80'
			/>
			<path
				className={`
					stroke-current
					${hamburgerPathStyle}
					path3
				`}
				stroke='#5757e0'
				strokeWidth='6.4px'
				d='M 20,70.999954 H 80.000231 C 80.000231,70.999954 94.498839,71.182648 94.532987,33.288669 94.543142,22.019327 90.966081,18.329754 85.259173,18.331003 79.552261,18.332249 75.000211,25.000058 75.000211,25.000058 L 25.000021,74.999942'
			/>
		</svg>
	);
}
export function Moon(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
			<path
				className='fill-current'
				fill='#5757E0'
				d='M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z'
			/>
		</svg>
	);
}
export function Sun(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
			<path
				className='fill-current'
				fill='#5757E0'
				d='M5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12ZM5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM12,5a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5Zm5.66,2.34a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34Zm-12-.29a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5ZM12,19a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19Z'
			/>
		</svg>
	);
}
export function Person(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 118 270'>
			<path
				fill='#FFB8B8'
				d='m5.6934 253.535 8.724 2.613 14.231-32.408-12.876-3.856-10.079 33.651Z'
			/>
			<path
				fill='#2F2E41'
				d='m4.3197 250.02 17.1816 5.146a11.4326 11.4326 0 0 1 6.782 5.526 11.4311 11.4311 0 0 1 .888 8.703l-.1066.356-28.1311-8.426 3.3861-11.305Zm16.0022-55.152-14.857 49.029 17.8279 5.2 17.086-45.315-20.0569-8.914Z'
			/>
			<path
				fill='#FFB8B8'
				d='m88.6017 260.663 8.5511-3.135-8.023-34.473-12.62 4.627 12.0919 32.981Z'
			/>
			<path
				fill='#2F2E41'
				d='m85.3984 258.671 16.8396-6.175a11.4316 11.4316 0 0 1 8.741.362 11.426 11.426 0 0 1 5.925 6.435l.128.349-27.5709 10.109-4.0627-11.08Zm-18.2773-52.66 13.372 46.057 18.571-8.914-13.371-43.086-18.572 5.943Z'
			/>
			<path
				fill='#3F3D56'
				d='M60.1626 214.318a203.998 203.998 0 0 1-35.4576-3.27l-.298-.06v-11.33l-6.7898-.755 5.2998-18.928c-2.4702-29.16 1.0399-66.089 2.1756-76.718.2603-2.493.4315-3.8916.4315-3.8916l5.9586-50.6496 9.9703-9.2031 4.5293 2.9475 8.4018 8.4015c9.766 24.0342 17.5155 46.661 17.5643 48.1148L104.7 203.93l-.221.156c-11.3724 8.028-28.7082 10.232-44.3164 10.232Z'
			/>
			<path
				fill='#000'
				opacity='.2'
				d='m35.708 71.1807-2.29 14.294 15.503 6.395-13.213-20.689ZM25.8926 98.667a27.8818 27.8818 0 0 0 15.5773 9.105 27.8877 27.8877 0 0 0 17.8908-2.34l3.3035-1.635-36.7716-5.13Z'
			/>
			<path
				fill='#2F2E41'
				d='M66.1288 39.4373H30.2849a2.7841 2.7841 0 0 1-2.781-2.7811V21.2063a20.7031 20.7031 0 0 1 35.3278-14.613 20.7031 20.7031 0 0 1 6.0781 14.613v15.4499a2.784 2.784 0 0 1-2.781 2.7811Z'
			/>
			<path
				fill='#FFB8B8'
				d='M52.1631 38.5068c8.3829 0 15.1786-6.7958 15.1786-15.1787 0-8.383-6.7957-15.1787-15.1786-15.1787-8.383 0-15.1787 6.7957-15.1787 15.1787 0 8.3829 6.7957 15.1787 15.1787 15.1787Z'
			/>
			<path
				fill='#2F2E41'
				d='M73.8068 22.7515H51.8891l-.2248-3.1468-1.1237 3.1468h-3.3749l-.4454-6.2368-2.2272 6.2368h-6.5302v-.3091a16.3954 16.3954 0 0 1 16.3768-16.377h3.0901a16.3952 16.3952 0 0 1 16.377 16.377v.3091Z'
			/>
			<path
				fill='#2F2E41'
				d='M51.7092 42.3215a2.839 2.839 0 0 1-.4921-.0435L35.168 39.4463V12.9219h17.667l-.4374.51c-6.0856 7.0973-1.5008 18.6057 1.7737 24.834a2.7394 2.7394 0 0 1-.2177 2.9089 2.7705 2.7705 0 0 1-2.2444 1.1467Z'
			/>
			<path
				className='fill-current'
				fill='#576EE0'
				d='M80.0279 106.433h-8.7972a1.1289 1.1289 0 0 1-.7637-.293 1.133 1.133 0 0 1-.3663-.732l-1.7609-18.0486h14.579l-1.7609 18.0486a1.1328 1.1328 0 0 1-.3662.732c-.209.19-.4815.294-.7638.293Z'
			/>
			<path
				fill='#2F2E41'
				d='M82.8964 89.6303H68.3639a1.1365 1.1365 0 0 1-1.1354-1.1354v-2.7248a1.1366 1.1366 0 0 1 1.1354-1.1353h14.5325a1.1366 1.1366 0 0 1 1.1353 1.1353v2.7248a1.137 1.137 0 0 1-1.1353 1.1354Z'
			/>
			<path
				fill='#FFB8B8'
				d='M78.7103 95.1734a6.9659 6.9659 0 0 0-10.6764.3224l-15.3256-4.3024-4.886 8.6756 21.728 5.772a7.003 7.003 0 0 0 4.8361 1.171 7.0058 7.0058 0 0 0 4.3933-2.336 7.0036 7.0036 0 0 0-.0694-9.3026Z'
			/>
			<path
				fill='#3F3D56'
				d='M42.1378 104.801c-7.2865.001-17.1453-4.271-28.8489-12.546a5.732 5.732 0 0 1-2.4134-3.9276c-.8632-5.4692 4.4715-12.863 4.9946-13.5718l5.6081-15.4005c.0646-.2503 1.8718-6.9136 6.409-9.2844a7.4391 7.4391 0 0 1 6.2139-.2648c8.6423 3.147 1.894 27.4482.9675 30.6351l11.4502 5.3894 7.2712 4.6349 9.9577 1.0417-2.7036 12.512-15.1227.34c-1.2381.303-2.509.452-3.7836.442Z'
			/>
		</svg>
	);
}
export function CSS(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16' fill='none'>
			<path
				className='fill-current'
				fill='#5757E0'
				fillRule='evenodd'
				clipRule='evenodd'
				d='M2.275 14.4 1 0h14l-1.277 14.401L8 16l-5.725-1.6Zm9.838-7.882.161-1.809.158-1.765H3.568l.115 1.669 6.84-.019-.132 1.794-6.533.077.176 1.676 6.208-.017-.269 2.451L8 11.107v.001l-1.972-.532-.132-1.471H4.12l.253 2.836 3.625 1.004.003-.001 3.627-1.003.486-5.423Z'
			/>
		</svg>
	);
}
export function Devices(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'>
			<path
				className='fill-current'
				fill='#5757e0'
				d='M28 64H18a8 8 0 0 1-8-8V34a8 8 0 0 1 8-8h10a8 8 0 0 1 8 8v22a8 8 0 0 1-8 8zM18 30a4 4 0 0 0-4 4v22a4 4 0 0 0 4 4h10a4 4 0 0 0 4-4V34a4 4 0 0 0-4-4z'
			/>
			<path
				className='fill-current'
				fill='#5757e0'
				d='M23 58a2 2 0 0 1-1.42-3.41 2.06 2.06 0 0 1 2.83 0A2 2 0 0 1 23 58zm-11-4H8a8 8 0 0 1-8-8V20a8 8 0 0 1 8-8h16a8 8 0 0 1 8 8v8h-4v-8a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v26a4 4 0 0 0 4 4h4z'
			/>
			<path
				className='fill-current'
				fill='#5757e0'
				d='M56 43H34v-4h22a4 4 0 0 0 4-4V8a4 4 0 0 0-4-4H16a4 4 0 0 0-4 4v7H8V8a8 8 0 0 1 8-8h40a8 8 0 0 1 8 8v27a8 8 0 0 1-8 8zm-12 8H34a2 2 0 0 1 0-4h10a2 2 0 0 1 0 4z'
			/>
			<path
				className='fill-current'
				fill='#5757e0'
				d='M40 51a2 2 0 0 1-2-2v-8a2 2 0 0 1 4 0v8a2 2 0 0 1-2 2z'
			/>
		</svg>
	);
}
export function Framer(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 256 256'>
			<polygon
				className='stroke-current'
				fill='none'
				stroke='#5757e0'
				strokeLinecap='round'
				strokeLinejoin='round'
				strokeWidth='20'
				points='56 100 56 168 128 236 128 168 200 168 56 32 200 32 200 100 56 100'
			/>
		</svg>
	);
}
export function HTML(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1792 1792'>
			<path
				className='fill-current'
				fill='#5757e0'
				d='m1322 597 16-175H454l47 534h612l-22 228-197 53-196-53-13-140H510l22 278 362 100h4v-1l359-99 50-544H663l-15-181h674zM192 128h1408l-128 1438-578 162-574-162z'
			/>
		</svg>
	);
}
export function React(props: AppProps) {
	const { svg } = props;
	function ellipsePreset(transform?: string) {
		return (
			<ellipse
				className='stroke-current'
				cx='12'
				cy='12'
				rx='5'
				ry='11'
				transform={transform}
				fill='none'
				stroke='#5757E0'
				strokeWidth='1.625'
			/>
		);
	}
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
			{ellipsePreset()}
			{ellipsePreset('rotate(-60 12 12)')}
			{ellipsePreset('rotate(60 12 12)')}
			<circle className='fill-current' cx='12' cy='12' r='2.5' fill='#5757E0' />
		</svg>
	);
}
export function TailwindCSS(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'>
			<path
				className='fill-current'
				fill='#5757e0'
				d='M18.5 9.51a4.22 4.22 0 0 1-1.91-1.34A5.77 5.77 0 0 0 12 6a4.72 4.72 0 0 0-5 4 3.23 3.23 0 0 1 3.5-1.49 4.32 4.32 0 0 1 1.91 1.35A5.77 5.77 0 0 0 17 12a4.72 4.72 0 0 0 5-4 3.2 3.2 0 0 1-3.5 1.51zm-13 4.98a4.22 4.22 0 0 1 1.91 1.34A5.77 5.77 0 0 0 12 18a4.72 4.72 0 0 0 5-4 3.23 3.23 0 0 1-3.5 1.49 4.32 4.32 0 0 1-1.91-1.35A5.8 5.8 0 0 0 7 12a4.72 4.72 0 0 0-5 4 3.2 3.2 0 0 1 3.5-1.51z'
			/>
		</svg>
	);
}
export function TypeScript(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 128 128'>
			<path
				className='fill-current'
				fill='#5757e0'
				d='M2,63.91v62.5H127V1.41H2Zm100.73-5a15.56,15.56,0,0,1,7.82,4.5,20.58,20.58,0,0,1,3,4c0,.16-5.4,3.81-8.69,5.85-.12.08-.6-.44-1.13-1.23a7.09,7.09,0,0,0-5.87-3.53c-3.79-.26-6.23,1.73-6.21,5a4.58,4.58,0,0,0,.54,2.34c.83,1.73,2.38,2.76,7.24,4.86,8.95,3.85,12.78,6.39,15.16,10,2.66,4,3.25,10.46,1.45,15.24-2,5.2-6.9,8.73-13.83,9.9a38.32,38.32,0,0,1-9.52-.1A23,23,0,0,1,80,109.19c-1.15-1.27-3.39-4.58-3.25-4.82a9.34,9.34,0,0,1,1.15-.73L82.5,101l3.59-2.08.75,1.11a16.78,16.78,0,0,0,4.74,4.54c4,2.1,9.46,1.81,12.16-.62a5.43,5.43,0,0,0,.69-6.92c-1-1.39-3-2.56-8.59-5-6.45-2.78-9.23-4.5-11.77-7.24a16.48,16.48,0,0,1-3.43-6.25,25,25,0,0,1-.22-8c1.33-6.23,6-10.58,12.82-11.87A31.66,31.66,0,0,1,102.73,58.93ZM73.39,64.15l0,5.12H57.16V115.5H45.65V69.26H29.38v-5a49.19,49.19,0,0,1,.14-5.16c.06-.08,10-.12,22-.1L73.33,59Z'
			/>
		</svg>
	);
}
export function UX(props: AppProps) {
	const { svg } = props;
	return (
		<svg className={svg} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 48 48' fill='none'>
			<path
				className='fill-current'
				fill='#5757E0'
				fillRule='evenodd'
				clipRule='evenodd'
				d='M6.34 6.34C4 8.69 4 12.46 4 20v8.095C4 35.572 4 39.33 6.34 41.66 8.69 44 12.46 44 20 44h8.095c7.477 0 11.235 0 13.565-2.34C44 39.31 44 35.54 44 28v-8.095c0-7.477 0-11.235-2.34-13.565C39.31 4 35.54 4 28 4h-8.095C12.428 4 8.67 4 6.34 6.34Zm10.16 7.91h15a2.25 2.25 0 0 1 2.25 2.25v2.75h-19.5V16.5a2.25 2.25 0 0 1 2.25-2.25Zm1 3.5h1a.75.75 0 1 0 0-1.5h-1a.75.75 0 1 0 0 1.5Zm3 0h1a.75.75 0 1 0 0-1.5h-1a.75.75 0 1 0 0 1.5Zm-5.591 15.341a2.25 2.25 0 0 1-.659-1.591V20.75h19.5V31.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-1.591-.659Zm9.827-9.148a.766.766 0 0 0 .184.557L27 27l-2.08 2.52a.76.76 0 0 0 .1 1.06.76.76 0 0 0 1.06-.1L28 28.17l1.94 2.31a.758.758 0 0 0 1.06.1.76.76 0 0 0 .1-1.06L29 27l2.08-2.48a.765.765 0 1 0-1.16-1L28 25.83l-1.92-2.33a.766.766 0 0 0-1.344.443Zm-7.766-.473a.75.75 0 0 0-.22.53v3.51a3.25 3.25 0 1 0 6.5 0V24a.75.75 0 1 0-1.5 0v3.51a1.75 1.75 0 0 1-3.5 0V24a.75.75 0 0 0-1.28-.53Z'
			/>
		</svg>
	);
}
