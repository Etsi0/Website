	...
	return (
		<div className='grid gap-x-4 gap-y-8' style={style}>
			{items.map((item, index) => (
				...
				>
					<summary className={cn('grid place-content-center col-[calc(var(--tabs-index)+1)] row-1 cursor-pointer select-none text-center bg-body-100 py-[0.75em] corner-shape-3.5 border-body-100 z-1 transition-colors duration-200 dark:bg-body-900 dark:border-body-800', clazz)}>{item.title}</summary>
					{item.content}
				</details>
			))}
		</div>
